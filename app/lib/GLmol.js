...
var GLmol = ( function() {
        function GLmol(id, suppressAutoload, force2d) {
            if (id)
                this.create(id, suppressAutoload, force2d);
            return true;
        }
        ...
        GLmol.prototype.defineRepresentation = function() {
            ...
            //this.drawAtomsAsSphere(this.modelGroup, hetatm, this.sphereRadius/5);
        ...
        GLmol.prototype.zoomInto = function(atomlist, keepSlab) {
            ...
            //(tmp[0][0] + tmp[1][0]) / 2, (tmp[0][1] + tmp[1][1]) / 2, (tmp[0][2] + tmp[1][2]) / 2);
            if (this.protein.appliedMatrix) {
                center = center.applyQuaternion(this.protein.appliedMatrix);
                //.multiplyVector3(center);
            }
        ...
        GLmol.prototype.render2d = function() {
            ...
            for (var i = 0, ilim = this.atoms.length; i < ilim; i++) {
                ...
                if (atom.screen == undefined)
                    atom.screen = new THREE.Vector3;
                atom.screen.set(atom.x, atom.y, atom.z);
                /*p*/
                mvMat.multiplyVector3(atom.screen);
                if (!this.webglFailed)
                    atom.screen.y *= -1;
                // plus direction of y-axis: up in OpenGL, down in Canvas
            ...
            for (var i = 0, ilim = toDraw.length; i < ilim; i++) {
                ...
                if (!toDraw[i][0]) {
                    ctx.fillStyle = "rgb(" + (atom.color >> 16) + "," + (atom.color >> 8 & 255) + "," + (atom.color & 255) + ")";
                    ctx.lineWidth = 0.03;
                    ctx.beginPath();
                    ...
                    ctx.strokeStyle = "rgb(" + (atom.color >> 16) + "," + (atom.color >> 8 & 255) + "," + (atom.color & 255) + ")";
                    ctx.beginPath();
                    ctx.moveTo(atom.screen.x, atom.screen.y);
                    ctx.lineTo(cx, cy);
                    ctx.closePath();
                    ctx.stroke();
                    ctx.strokeStyle = "rgb(" + (atom2.color >> 16) + "," + (atom2.color >> 8 & 255) + "," + (atom2.color & 255) + ")";
                    ctx.beginPath();
...
THREE.TrackballControls = function(object, domElement) {
    ...
    this.getMouseOnScreen = function(pageX, pageY, optionalTarget) {
        ...
        return (optionalTarget || new THREE.Vector2() ).set((pageX - _this.screen.left ) / _this.screen.width, (pageY - _this.screen.top ) / _this.screen.height);
    ...
    this.getMouseProjectionOnBall = ( function() {
            ...
            return function(pageX, pageY, projection) {
                ...
                var mouseOnBall = new THREE.Vector3((pageX - _this.screen.width * 0.5 - _this.screen.left ) / (_this.screen.width * .5), (_this.screen.height * 0.5 + _this.screen.top - pageY ) / (_this.screen.height * .5), 0.0);
